...
class Database:
    ...
    async def add_tgfiles(self, chat_id, file_id, hash, name, size, file_type):
        if fetch_old := self.files.find_one({"chat_id": chat_id, "hash": hash, "title": name}):
            return
    ...
    async def search_static_tgfiles(self, id, name):
        print(id, type(id), name, type(name))
        query = {'chat_id': id, 'title': name}
        mydoc = self.files.find(query)
        return list(mydoc)
